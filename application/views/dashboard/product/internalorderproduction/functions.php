...
function setProduction(order_id, el) {
    $.ajax({
        url: "<?=base_url('product/getproductiondata?id=')?>"+order_id,
        method: "POST",
        dataType: 'text', 
        success: function(res) {
            swal({
                title: "Are you sure?",
                text: "Confirm Production at " + order_id + ".",
                type: "warning",
                showCancelButton: true,
                confirmButtonClass: "btn-warning",
                cancelButtonText: "No, cancel plx!",
            ...
            function(isconfirm) {
                ...
                const setproducted = ((res==0)?1:0);
                const form_data = {
                    isproducted: setproducted
                }
                $.ajax({
                    url: "<?=base_url('product/setproduction?id=')?>"+order_id,
                    method: "POST",
                    data: form_data, 
                    dataType: 'json', 
                    success: function(res) {
                        ...
                        if (setproducted==1) {
                            $(etd[8]).html("<label class='status'><?=date('Y/m/d')?></label>");
                            $(etd[9]).html("<button onclick='setProduction("+order_id+", this)'><i class='bi bi-dash'></i></button>");
                        } else {
                            $(etd[8]).html("<label class='status'>Not Producted</label>");
                            $(etd[9]).html("<button onclick='setProduction("+order_id+", this)'><i class='bi bi-check-all'></i></button>");
                        }
